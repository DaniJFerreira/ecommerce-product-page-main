...
//   // Fetch images and initialize the gallery
//   fetch(imageJason)
...
//       })
//       .catch(console.error); // Handle errors in fetching JSON data
...
createGalleryItems(images);
  ...
  function openModal(index) {
    if (index >= 0 && index < images.length && images[index].id === 'Main-image'){
      currentIndex = index ;
      updateModalImage();
  ...
  function updateModalImage() {
      ...
      const newSelectedImgElem = galleryElement.querySelector(`[data-index="${currentIndex}"]`);
  ...
  document.querySelector('.Gprev').addEventListener('click', () => changeImage(-1));
  document.querySelector('.Gnext').addEventListener('click', () => changeImage(1));
