...
export function showCart() {
    ...
    function handleClickOnPage(event) {
        ...
        } else if (!openCart.contains(event.target) && 
...
const findProductById = (id) => products.find(product => product.id.toString() === id.toString());
        ...
        cart.forEach(item => {
            ...
            let newItemHTML = `
                <div class="item" data-id="${item.product_id}">
                    <div class="image">
                        <img src="${product.image ?? 'default-placeholder.png'}">
                    </div>
                    ...
                    <div class="totalPrice"><p>$${(product.price * item.quantity).toFixed(2)}</p></div>
